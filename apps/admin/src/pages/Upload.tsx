...
export default function Upload() {
  ...
  const onFile = async (f: File) => {
    ...
    try {
      ...
      if (cy.cycles?.length) {
        ...
        setStartCycle(last);
        setEndCycle(last); // varsayılan: son cycle → son cycle
      }
    } catch (e: any) {
  ...
  const onCompute = async () => {
    ...
    try {
      const b = await briefAPI(file, startCycle, endCycle ?? startCycle);
      setBrief(b);
    } catch (e: any) {
  ...
  return (
    <div>
      {/* Üst grid: Yükleme + Cycle aralığı */}
          ...
          {cycles && cycles.length > 0 && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 12 }}>
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setStartCycle(v);
                    // Bitiş başlangıçtan küçük olamaz → gerekiyorsa düzelt
                    if (v !== null && (endCycle === null || (endCycle as number) < v)) {
                      setEndCycle(v);
                    }
                  }}
              ...
              <div>
                ...
                >
                  {cycles
                    .filter(c => startCycle === null || c.index >= startCycle)
                    .map(c => <option key={c.index} value={c.index}>{c.label}</option>)}
                </select>
              </div>
            </div>
          )}
