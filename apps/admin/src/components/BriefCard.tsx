...
type GameLine = { game_name: string; wager: number; profit: number; ggr: number };
type Row5 = { items: GameLine[] };
type Row6 = { items: GameLine[] };
...
type Brief = {
  ...
  row5_top_games?: Row5; // backward compat
  currency?: string|null;
};

export default function BriefCard({ data }: { data: Brief }) {
  const r1 = data.row1_last_op, r2 = data.row2_wager, r3 = data.row3_open, r4 = data.row4_late;
  ...
  const cycleLabel =
    typeof data.cycle_index_from === "number" && typeof data.cycle_index_to === "number"
      ? (data.cycle_index_from === data.cycle_index_to ? `#${data.cycle_index_from}` : `#${data.cycle_index_from} → #${data.cycle_index_to}`)
  ...
  return (
    <div style={wrap}>
      ...
      {/* 1) Son İşlem & Kaynak — Sıra: Tür → Tutar → Tarih → Kaynak/Detay → Bonus Türü */}
      <div style={row}>
        <div style={box}>
          <div style={cap}>1) Son İşlem & Kaynak</div>
          <div style={{ display: "grid", gridTemplateColumns: "180px 1fr 1fr 1fr 1fr", gap: 12 }}>
            ...
            <div><b>Kaynak/Detay</b><div style={{ opacity: 0.95 }}>{r1.type === "BONUS" ? (r1.bonus_detail || "-") : (r1.method || "-")}</div></div>
            <div><b>Bonus Türü</b><div>{r1.type === "BONUS" ? (r1.bonus_kind || "other") : "-"}</div></div>
          </div>
        </div>
      </div>

      {/* 2) Toplam Çevrim (seçili cycle/aralık) */}
      <div style={row}>
        <div style={box}>
          <div style={cap}>2) Toplam Çevrim (Seçili Aralık)</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
            <div><b>Başlangıç</b><div>{r2.window_from}</div></div>
            <div><b>Bitiş</b><div>{r2.window_to || "—"}</div></div>
            <div><b>Bahis (Toplam Çevrim)</b><div>{fmt(r2.wager_total)} <span style={{ fontSize: 12, color: "#94a3b8" }}>({r2.wager_count} bahis)</span></div></div>
          </div>
        </div>
      </div>
      ...
      {/* 5) En Çok Çevrim Yapılan 3 Oyun — Bahis / Kazanç / GGR */}
      <div style={row}>
        <div style={box}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap: 12 }}>
              {r5.items.map((g, i) => (
                <div key={`w${i}`} style={{ border:"1px dashed #263244", borderRadius:10, padding:10 }}>
                  ...
                  <div>Bahis: {fmt(g.wager)}</div>
                  <div>Kazanç: <b style={{ color: g.profit >= 0 ? "#34d399" : "#f87171" }}>{fmt(g.profit)}</b></div>
                  <div>GGR: <b>{fmt(g.ggr)}</b></div>
                </div>
              ))}
            </div>
      ...
      {/* 6) En Çok Kâr Eden 3 Oyun — Bahis / Kazanç / GGR */}
      {r6.items?.length ? (
        <div style={row}>
          <div style={box}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap: 12 }}>
              {r6.items.map((g, i) => (
                <div key={`p${i}`} style={{ border:"1px solid #1f2937", borderRadius:10, padding:10, background:"#121824" }}>
                  ...
                  <div>Bahis: {fmt(g.wager)}</div>
                  <div>Kazanç: <b style={{ color: "#34d399" }}>{fmt(g.profit)}</b></div>
                  <div>GGR: <b>{fmt(g.ggr)}</b></div>
                </div>
              ))}
            </div>
